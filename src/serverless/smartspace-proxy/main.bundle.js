...
var SMARTSPACE_APP_ID = process.env.SMARTSPACE_API_APP_ID;
var SMARTSPACE_WORKSPACE_ID = process.env.SMARTSPACE_WORKSPACE_ID;
      ...
      try {
        ...
        for (const msg of messages) {
          const hasOutput = msg.values?.some(
            (v) => (v.type === "Output" || String(v.type) === "2") && v.name === "Response"
          );
          if (hasOutput) {
        ...
        const userMsgTimestamp = lastUserMessageTime ? new Date(lastUserMessageTime).getTime() : 0;
        const isFresh = msgTimestamp > userMsgTimestamp - 15e3;
